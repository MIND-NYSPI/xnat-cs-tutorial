...
You might wonder why there is no matcher for the file.  There should be, and the most logical matcher would be "'nii' in @.name".  However, this yields no results and the command doesn't run.  So provisionally, I have been running this command with no matcher, but that's not a good long-term solution.
...
One more detail to watch out for -- when you specify an output file, you need to tell XNAT where in your mounted directory to put it.` "path": "./"` tells XNAT that the file belongs in the top level of the directory.  
